...
namespace PocketMoney.Util.Encoding
{
    ...
    public class Base32Url
    {
        ...
        private static readonly Dictionary<string, Dictionary<string, uint>> _indexes =
        ...
        public Base32Url(bool padding, bool caseSensitive, bool ignoreWhiteSpaceWhenDecoding, string alternateAlphabet)
        {
            if (alternateAlphabet.Length != 32)
            {
                throw new ArgumentException(
                    String.Format("Alphabet must be exactly 32 characters long for base 32 encoding. Was: {0}",
                                  alternateAlphabet)).LogError();
            }
        ...
        {
            if (String.IsNullOrEmpty(input)) throw new ArgumentNullException("'input' parameter is null or empty");
            ...
            if (UsePadding)
            {
                if (input.Length%8 != 0)
                {
                    throw new ArgumentException("Invalid length for a base32 string with padding.").LogError();
                }
                ...
                for (int charOffset = 0; charOffset < chars; charOffset++)
                {
                    ...
                    if (!_index.TryGetValue(input.Substring(i + charOffset, 1), out cbyte))
                    {
                        throw new ArgumentException("Invalid character '" + input.Substring(i + charOffset, 1) +
                                                    "' in base32 string, valid characters are: " + _alphabet).LogError();
                    }
